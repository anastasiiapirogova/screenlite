...
import { FolderRestoreService } from '../FolderRestoreService.ts'
...
describe('FolderRestoreService tests', () => {
    ...
    it('should restore deleted folders', async () => {
        ...
        const restoredFolder1 = await prisma.folder.findUnique({ where: { id: folder1.id } })
        const restoredFolder2 = await prisma.folder.findUnique({ where: { id: folder2.id } })
    ...
    it('should handle already restored folders', async () => {
        ...
        const activeFolder = await prisma.folder.create({
            data: {
    ...
    it('should restore folders with parent relationships', async () => {
        ...
        const restoredFolder = await prisma.folder.findUnique({ where: { id: childFolder.id } })
    ...
    it('should handle deleted parent folders when restoring folders', async () => {
        const deletedParentFolder = await prisma.folder.create({
            data: {
    ...
    it('should restore nested folders and their files', async () => {
        ...
        const restoredParentFolder = await prisma.folder.findUnique({ where: { id: parentFolder.id } })
        const restoredChildFolder = await prisma.folder.findUnique({ where: { id: childFolder.id } })
